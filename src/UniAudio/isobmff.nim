...
iterator boxes*(data: string; start, limit: int): tuple[kind: string;
  ...
  # The caller's limit bounds the nesting but is not trusted for reach: a
  # parent span may name more bytes than the data holds, and a span yielded
  # past its end would be read out of bounds by whoever asked for it.
  ...
  while offset >= 0 and offset + 8 <= effective:
    ...
    var header = 8
    if size == 1:
      if offset + 16 > effective: break
      size = beU64(data, offset + 8)
      header = 16
    elif size == 0:
      size = int64(effective - offset)
...
proc sampleData*(data: string; track: AudioTrack; index: int): string
  ...
  ##
  ## The index is the caller's, so it is checked in the body rather than by a
  ## precondition: a precondition compiles away under `-d:release`, and the
  ## release build is the one a consumer ships.
  body:
    if index < 0 or index >= track.sizes.len:
      raise newException(AudioError, "mp4: no such sample")
